...
function goBuy(id)
{
	...
	location.href = "/easybuy/shopping/addDetail?pid="+id;
}
	...
	if (window.XMLHttpRequest)
 	{// code for Firefox, Opera, IE7, etc.
  	...
  	{// code for IE6, IE5
		...
		xhr.open("post", "/easybuy/shopping/delDetail?did="+id,true);
	...
	if (window.XMLHttpRequest)
 	{// code for Firefox, Opera, IE7, etc.
  	...
  	{// code for IE6, IE5
		...
		xhr.open("post", "/easybuy/shopping/modifyDetail?did="+id+"&count="+number.value,true);
		xhr.onreadystatechange=function(){
			if(xhr.readyState==4&&xhr.status==200)
			{
				//alert(xhr.responseText);
...
{
	var newCookie = "";
	var oldCookie = getCookie("pid");
	if(oldCookie) {
		if(inArray(oldCookie.split(","), pid)) {
			newCookie = oldCookie;
		} else {
			...
			if(array.length>=5)
			{
				...
				newCookie = pid + "," + oldCookie;
			}
		}
	} else {
		newCookie = pid;
	}
	setCookie("pid", newCookie);
	//alert(newCookie);
}